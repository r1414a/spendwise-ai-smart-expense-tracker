export default async function CoreMetrics() {
  ...
  console.log("core metrics: ", result);
  return (
    <section className="max-w-screen-xl mx-auto pt-40 pb-32 px-4 md:px-32">
      <div className="flex flex-col lg:flex-row gap-10">
        {Object.entries(result).map(([key, value]) => (
          <div key={key} className="z-30 relative basis-1/2 p-8 md:p-10 bg-white rounded-lg border border-lightgray/40 shadow-lg shadow-lightgray/20 flex flex-col gap-6 justify-start items-center">
            ...
            <div className="space-y-3 w-full">
              <ul className="flex gap-2">
                {Object.entries(value.top3Categories).map(([key, value]) => (
                  <li
                    key={key}
                    className="bg-lightblue p-2 sm:p-2.5 rounded-md flex-1 space-y-1"
                  >
